...
/**
 * Servlet implementation class ExerciseForm
 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (Utils.isNumeric(idString)) {
			...
			if (id < 0) {
				...
				session.setAttribute("exerciseId", idString);
				request.getRequestDispatcher("/WEB-INF/panel/ExerciseForm.jsp")
				.forward(request, response);
			}
			
		} else {
			writer.append("Id must be a integer!<br>");
			writer.append("<a href='ExerciseAdmin'>Return</a>");
